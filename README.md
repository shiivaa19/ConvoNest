...
ConvoNest is a modern, full-stack, real-time chat application built with React.js, Node.js, Express.js and powered by Socket.IO. The platform enables users to register, log in, update their profiles, and engage in private, one-on-one conversations with text and images. The backend uses PostgreSQL for robust data management.
...
| **Database** | PostgreSQL (Railway), pg (Node PostgreSQL driver)                                                                                     |
...
- npm or yarn
- A [Railway](https://railway.app/) account with PostgreSQL database (or any PostgreSQL database).
